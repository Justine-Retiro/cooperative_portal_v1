...
function toggleSidebar() {
    ...
    if ($("#wrapper").hasClass("toggled-2")) {
        ...
        if ($(window).width() <= 768) { // MD breakpoint check
            $("#card-container").removeClass("dashboard-card");
            $(".col-sm-12").removeClass("dashboard-cards");
            $(".card-whole").css("width", "");
        }

        $(".toggle-menu").removeClass("d-none");
    } else {
        if ($(window).width() <= 768) { // MD breakpoint check
            $("#card-container").addClass("dashboard-card");
            $(".col-sm-12").addClass("dashboard-cards");
            $(".card-whole").css("width", "inherit");
        }
